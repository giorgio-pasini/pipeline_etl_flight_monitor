...
### Rétention & nettoyage
La rétention reste **triviale** grâce au partitionnement horodaté : purger = supprimer les
répertoires de partitions antérieurs au seuil voulu (en prod, lifecycle policy de l'object
storage S3/GCS). Le tuning Spark (mémoire, shuffle) se règle via `PipelineConfig`/variables
d'env (§6).
...
- **`max_workers=3`** : concurrence réduite du client API.
- Garde externe `retry_with_backoff` sur l'appel feed (un échec n'annule pas la zone).
...
Chemins datalake, paramètres Spark, API (timeout, `API_MAX_WORKERS_PARALLEL`,
`API_MAX_RETRIES`, `API_RETRY_*`), `FR24_EMAIL/PASSWORD`,
...
- **Code** : modules `src/` (schemas, data_quality, flight_extraction, batch_job,
  ...
  (run_job, init_datalake), `dashboard.py`.
